...
export function ProductTable({ products: initial }: ProductTableProps) {
  ...
  return (
    <div className="bg-white rounded-2xl p-8 border border-rose-gold/15 shadow-[var(--shadow-card)]">
      <h2 className="font-serif text-3xl text-charcoal mb-2">Recent products</h2>
      ...
      {products.length === 0 ? (
        ...
        <ul className="divide-y divide-rose-gold/10">
          {products.map((p) => (
            ...
            >
              <div className="relative w-16 h-16 rounded-xl bg-mist overflow-hidden flex-shrink-0">
                {p.imageUrls[0] && (
                  <Image
                    src={p.imageUrls[0]}
                    alt={p.title}
              ...
              <div className="flex-1 min-w-0">
                <Link
                  ...
                  className="font-serif text-lg text-charcoal hover:text-rose-gold transition-colors line-clamp-1"
                >
                  {p.title}
                </Link>
                <p className="text-xs text-warm-gray font-sans tracking-wide">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  disabled={busyId === p.id}
                  className={`text-xs uppercase tracking-[0.2em] font-sans px-3 py-1 rounded-full border transition-all ${
                    p.published
